...
- **과차단 4건 미수정** — `ShapeCoverageTest`의 `F2`·`H6`(겹 경계에서 멈춤: CTE 안 테이블 + 바깥 조건),
  `G1`·`G5`(조건 조각 미합산: 부등호 둘 = BETWEEN, OR 분배). 방향은 안전하나 **쿼리를 읽기 좋게
...
- **`ShapeCoverageTest`가 사용자 정의 규칙을 재지 않는다** — 픽스처에 룰 트리가 없다.
