...
class QueryFlowIntegrationTest {
    ...
    /**
     * spec 008 §2.6: 접수 위반은 **저장 게이트에서** 막힌다.
     * 실행 주석 안의 UNION은 룰 층이 볼 수 없으므로(§2.5-2), 접수 검사가 없으면 이 쿼리는 저장·승인까지 통과한다.
     */
    @Test
    @Order(6)
    fun `접수 위반 SQL은 저장되지 않는다`() {
        ...
        assertTrue(
            hidden.body!!["violations"].toString().contains("intake/comment-not-allowed"),
            "접수 위반으로 차단되어야 함: ${hidden.body}",
        )
        ...
        assertTrue(qualified.body!!["violations"].toString().contains("intake/schema-qualifier"))
    }
