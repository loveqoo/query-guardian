...
class ShapeCoverageTest {
    ...
    private val shapes: List<Shape> = listOf(
        ...
        // ── E. 마스킹 — 투영 위치와 비투영 위치 ───────────────────────────────────
        // spec 012 P2: 서버가 대신 가려주지 않는다 — 맨몸 투영은 차단이고 사용자가 직접 가려야 한다.
        Shape("E1", "마스킹", "마스킹 대상을 그대로 투영",
            "SELECT email FROM users LIMIT 10",
            Verdict.BLOCK, "서버가 대신 가리지 않는다. 정답은 `mask_email(email)`이고 그것은 M1이 잰다"),
        Shape("E2", "마스킹", "마스킹 대상을 개명해 투영",
            "SELECT email AS e FROM users LIMIT 10",
            Verdict.BLOCK, "개명해도 가려지지 않은 것은 같다 — E1과 같은 판정이어야 한다"),
        ...
        Shape("E4", "마스킹", "CTE에서 투영하고 바깥에서 그대로 넘기기",
            ...
            Verdict.BLOCK, "겹을 늘려도 가려지지 않은 것은 같다 — E1과 같아야 한다"),
        ...
        Shape("K13", "사용자 규칙", "must_be_masked — 맨몸 투영",
            "SELECT name FROM users WHERE created_at = '2026-01-01' LIMIT 10",
            Verdict.BLOCK, "사용자 규칙도 시스템 룰과 같은 판정이어야 한다 — 서버가 대신 가리지 않는다", rule = 4),
