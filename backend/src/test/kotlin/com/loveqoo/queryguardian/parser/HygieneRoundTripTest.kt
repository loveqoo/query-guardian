 ...
 *
 *     checkHygiene(sql) 통과  ⟹  checkHygiene(Druid가 출력한 sql) 통과
 *
...
class HygieneRoundTripTest {
    ...
    @Test
    fun `위생 통과 SQL은 프린터 출력도 위생을 통과한다`() {
        val violations = mutableListOf<String>()
        var checked = 0
        for (sql in corpus) {
            val before = parser.checkHygiene(sql)
            ...
            val after = parser.checkHygiene(printed)
            if (after.isNotEmpty()) {
    ...
    /** 코퍼스의 정상 쿼리가 위생 게이트에 걸리면 그것 자체가 오차단이다 — 어느 문장이 걸렸는지 드러낸다. */
    @Test
    fun `코퍼스의 정상 쿼리는 위생 위반이 없다`() {
        val rejected = corpus.mapNotNull { sql ->
            parser.checkHygiene(sql).takeIf { it.isNotEmpty() }?.let { "$sql → ${it.map { v -> v.code }}" }
        }
