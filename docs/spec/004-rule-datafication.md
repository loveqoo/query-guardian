...
- **deferred 조건(must_be_*, judged=false)은 만족/불만족 어느 쪽에도 계상하지 않고 severity 집계에서도 제외.**
  *(→ spec 008 M1-6에서 `must_be_masked`는 **판정 대상으로 전환**됐다: 재작성이 생겨 "마스킹 안 됨"을 정의할 수
  있게 됐기 때문. 전환은 **소급 효과**가 있다 — 그 조건만 가진 기존 규칙은 이 절의 "미강제(표시 전용)"에서
  강제 규칙으로 바뀌고, 표현 불가 위치 사용은 BLOCK이 된다. 판정 기준은 재작성 계획 수립기와 같은 함수
  (`ir/maskUsageOf`)를 공유해 "저장은 통과, 실행은 마스킹 없이"를 원리적으로 막는다. `must_be_within`은 여전히 미판정.)*
