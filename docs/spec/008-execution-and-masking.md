...
> 상태: **v2 (적대 검토 반영)** · ⚠️ **§0 정정을 먼저 읽을 것 — 이 스펙의 전제가 틀렸다**
...
## 0. 정정 (2026-07-27) — **자동 재작성은 사용자 요구가 아니었다**

### 무엇이 틀렸나

§11의 결정 2 **"자동 재작성 — MASK 치환·FILTER 주입을 서버가 수행"** 은
`(2026-07-25 사용자)` 머리글 아래 **AI 표시 없이** 적혀 있었다. 사용자 결정으로 읽힌다. **틀렸다.**

사용자에게 확인한 결과: **논의한 적 없다.** 디자인 화면에 마스킹된 값(`j***@naver.com`)이 그려져
있는 것을 보고 AI가 **"서버가 SQL을 고쳐서 마스킹한다"를 유추**했고, 그 유추를 사용자 결정으로 기록했다.
§11은 4번부터 `(파생·AI)`를 붙여 구분하고 있었으므로, **구분 규칙을 만들어 놓고 가장 큰 항목에서
지키지 않은 것**이다.

### 사용자가 밝힌 실제 모델

> "우리는 쿼리 작성을 도와줄 뿐, 작성은 사용자가 합니다.
> 마스킹이 필요하다면 룰로 등록하고 규제하면 그만입니다.
> 하지만 쿼리 추천은 해줄 수 있어야 합니다. 코드 추천해주듯이 말이죠."

| 축 | 이 스펙이 만든 것 | 실제 모델 |
|---|---|---|
| 마스킹 안 된 컬럼 조회 | 서버가 `mask_email()`로 **바꿔서 실행** | **위반으로 막고** 고칠 방법을 **제안** |
| 필수 조건 누락 | 서버가 `WHERE`에 **끼워 넣음** | **위반으로 막고** 추가할 조건을 **제안** |
| 실행 대상 | 재작성된 SQL | **사용자가 쓴 SQL 그대로** |

### 이 정정이 여는 것

- **판정 대상과 실행 대상이 갈라지는 위협이 사라진다.** §2.5-1이 실측한 그 위협을 막느라 쓴 복잡도
  (결정 13의 AST 핸들, `scopeId` 난스, `RewriteVerifier` 재검증)가 **문제 자체와 함께 없어진다.**
- ~~**행 상한은 `setMaxRows`로 옮길 수 있다.**~~ — **이 줄은 틀렸다**(2026-07-27 정정).
  spec 012 P1로 실제로 옮겼다가 되돌렸다: `setMaxRows`는 **JDBC 구현마다 보장이 다르고**
  이 제품은 멀티 벤더가 예정돼 있다. **IR을 통한 `LIMIT` 주입이 벤더 무관하게 서버를 멈추는
  유일한 방법**이므로 §3.0-2의 선택이 그대로 옳다(spec 012 §4, `QueryExecutor` KDoc의 실측).
- **재작성 엔진(706줄)은 버리는 것이 아니라 출력처를 바꾼다** — 실행 변환기에서 **제안 생성기**로.
  `preview-rewrite`가 이미 그 모양이다.
- **관계대수 IR(Apache Calcite) 도입이 다시 열린다.** 원문 형태를 보존할 이유가 약해지기 때문이다
  (스파이크 결과는 ADR 참조 — 우리가 손으로 다루는 표기 문제 대부분이 그쪽에서는 생기지 않는다).

### 남은 작업

이 정정은 **기록만** 한다. 실행 모델 변경은 별도 스펙(012)이며, 이 스펙의 M0~M2가 만든 것은
"그때 그렇게 만들었다"로 정확히 남긴다 — 되돌려 쓰지 않는다.
...
**게이트 순서 → §5.** 여기에 다시 적지 않는다. 예전에는 이 자리에 순서 사본이 있었고, §5가 코드 기준으로
정정된 뒤에도 사본은 낡은 채 "§5 확정"이라는 권위 표시를 달고 남아 있었다(spec 010 검토 F10).
**순서는 정책이므로 정의가 하나여야 한다**(spec 010 I1) — 사본은 반드시 늦게 썩고, 하필 그 낡은 배치가
룰 재판정을 요청자 일치보다 앞에 두는 위험한 쪽이었다.
...
## 5. 실행 게이트 (순서 — v3: 코드 기준으로 정정, spec 010 P0)

```
POST /api/queries/{id}/execute
인증(401) → 열람 권한(403 FORBIDDEN_READ, 본문 미기록)
→ 근거 승인 요청 존재(403 NO_REQUEST) → **요청자 == 세션 principal**(403 REQUESTER_MISMATCH)
→ 검토 APPROVED 확인(403 NOT_REVIEWED)
→ **파싱 1회**(실패 시 422 PARSE_FAILED) → 데이터 권한(403, **현재 권한**으로 재검사)
→ 승인 재검사(상태·요청자·테이블 커버, 403)
→ **접수 검사 + 룰 재판정**(현재 규칙·현재 카탈로그, 422 RULE_BLOCKED)
→ 데모 매핑 총체성(403) → 계획 → 재작성 → 재작성 검증(§3.0.3) → 행 상한 확인 → 실행
```
- **접수 위반과 룰 위반은 한 판정으로 합쳐 나온다**(`LintService.judge`가 `intakeViolations + engine.lint`).
  둘을 별도 단계로 적으면 안 된다 — 코드에 그런 단계가 없고, 분류 코드도 `RULE_BLOCKED` 하나다.
- **SQL 길이 검사는 이 경로에 없다.** 저장·lint·미리보기 진입점에만 있다(`RequestValidation.validateSql`).
  실행 대상은 저장 시점에 이미 검사된 SQL이라 오늘은 문제가 없지만, **없는 단계를 적지 않는다** —
  "문서에 있으니 있겠지"가 다음 구현자의 판단 근거가 된다.

```
POST /api/preview-rewrite  (저장 이전 단계 — 순서가 다르다)
인증(401) → 승인 요청 존재(403 NO_REQUEST) → **파싱 1회**(422)
→ 데이터 권한(403) → 승인 검사(상태·**요청자 일치**·테이블 커버, 403)
→ 접수 검사 + 룰 재판정(422) → 데모 매핑 총체성(403) → 계획 → 재작성 → 재작성 검증 → **PREVIEW 기록**
```
- 검토 상태는 보지 않는다(저장 이전이므로). 요청자 일치는 별도 단계가 아니라 승인 검사가 겸한다.
- 행 상한 확인·실행이 없다. 그래도 **카탈로그 오라클**이므로 감사 등급은 SUCCESS와 같다(spec 010 I4).
> **v2까지 이 절은 코드와 다른 순서를 적고 있었고, 문서 쪽이 더 위험했다**(spec 010 D-C).
> 룰 재판정을 요청자 일치보다 **앞**에 두면, STEWARD가 남의 쿼리 id로 실행을 시도하는 것만으로
> 그 쿼리의 판정 결과(위반 목록 = 어떤 컬럼이 PII·BLOCK인지)를 응답으로 받아 간다.
> 신원 검사는 **판정 결과를 만들기 전에** 끝나야 한다. 순서가 정책인 이상, 문서와 코드가 갈리면
> 다음 구현자가 "문서대로" 고쳐서 구멍을 낸다 — 그래서 코드 기준으로 맞췄다.
> 각 코드가 실제로 발생하고 감사에 남는지는 `AuditCodeCoverageTest`가 전수 검증한다(spec 010 A0).
...
- **결과 행은 저장하지 않는다**(불변식). `error_message`는 SQLState+vendor code+정제 메시지만 — MySQL 오류는
  데이터 값을 에코하므로(`Truncated incorrect ... value: '...'`) 사용자 응답에는 **분류 코드만** 반환하고 원문은 STEWARD/ADMIN에게만. 분류 코드의 값 집합은
  `audit.AuditCode`가 닫는다(spec 010 P0) — 예전에 여기 적혀 있던 `REWRITE_FAILED`는 **존재한 적이 없는 코드**다.
...
```
POST /api/queries/{id}/execute        → 200 ExecutionResultDto | 403(권한·미검토·**재작성 실패**) | 422(룰·실행 오류)
       ※ **재작성 실패는 현행 403이다**(권한 실패가 아닌데도). 이 표는 오래 422라고 적고 있었고
         실측으로 어긋남이 확인됐다(`AuditCodeCoverageTest`). 응답 코드 통합은 spec 010 P1(A2)에서 —
         그때까지 **문서는 현행을 적는다**(있는 그대로가 아니면 안전망이 아니다).
...
### 8-1. M3 결정 (2026-07-26 사용자) — 디자인과 모델이 부딪히는 세 자리

디자인(`dc.html` 239–368)은 에디터에서 결과를 바로 보여주고 상한을 한 줄로 뭉친다.
**결정 4**(임의 SQL 실행 비범위)와 **retrospect 012**(상한 3값을 뭉치지 말 것)에 걸린다. 셋을 이렇게 정한다.

1. **`규칙 검사` = lint + 재작성 미리보기, `실행` = 승인분만.** 버튼 셋(검사·실행·저장)은 디자인 그대로
   두고, `규칙 검사`가 `POST /api/preview-rewrite`까지 불러 **무엇이 자동 적용될지**를 같은 탭에 보인다
   (미리보기는 실행 게이트와 같은 검사를 지나므로 저장 전에 차단 사유도 같이 드러난다).
   `실행`은 저장·검토 승인된 쿼리를 에디터에서 열었을 때만 활성이고, 그 외에는 **비활성 + 이유**
   (미저장 / 검토 대기 / 반려). 기존 두 탭(규칙 검사 결과 · 실행 결과)에 그대로 대응된다.
2. **상한은 평소 한 줄, 잘렸을 때 경고.** 사용자 LIMIT과 거버넌스 상한이 같으면 디자인의 한 줄을 쓴다.
   거버넌스 상한이 실제로 자른 경우(`effectiveLimit == configuredCap` 이면서 요청이 더 컸을 때)
   경고 줄을 더한다. `moreRowsExist`는 **세 상태**다 — 있음 / 없음 / **확인하지 않음**(`null`).
   불리언에 "모름" 자리가 없으면 언젠가 거짓말한다(retrospect 014, `LIMIT 0`이 그 사례).
3. **실행 감사는 `거버넌스` 그룹의 새 화면**(STEWARD/ADMIN 전용, `GET /api/executions`).
   디자인에 없는 화면이므로 셸·표 관용구는 기존 화면을 답습한다. 쿼리별 이력만으로는
   **삭제된 쿼리와 PREVIEW 기록이 다시 안 보인다**(§6이 그 위협을 이유로 이 API를 만들었다).

### 8-2. 화면 쪽 불변식

| # | 불변식 | 왜 |
|---|---|---|
| F1 | **결과 행은 어디에도 영속되지 않는다** — `localStorage`·`sessionStorage`·URL·캐시 라이브러리 금지 | §6의 서버 불변식은 화면이 캐시하면 무의미해진다. 오늘 프론트에 캐싱 라이브러리가 없다는 것이 전제(실측) |
| F2 | **상한 3값을 한 값으로 합치지 않는다** | 합치면 감사와 화면의 어휘가 갈리고, "상한이 걸려 잘렸다"는 거짓을 남긴다 |
| F3 | **화면의 비활성은 서버 판정을 대체하지 않는다** | 버튼 disabled는 UX다. 권한·상태 판정은 서버가 하고, 서버가 거절하면 그 **분류 코드를 그대로** 보인다 |
| F4 | **오류 원문을 화면이 합성하지 않는다** | `errorDetail`은 서버가 역할에 따라 마스킹한다(§6). 화면이 만들면 그 마스킹이 무의미해진다 |
| F5 | **감사 필터는 닫힌 어휘**(`outcome`은 select) | 자유 입력이면 오타가 **빈 결과**가 되고(서버 fail-closed) 사용자는 "기록 없음"으로 읽는다 |
...
### 9-1. M3 수용 기준 (관측 가능한 것만)

| # | 기준 | 재는 방법 |
|---|---|---|
| M3-1 | 미저장·검토대기·반려 **세 상태에서 `실행`이 비활성이고 이유가 화면에 있다** | E2E 3케이스 |
| M3-2 | `규칙 검사`가 자동 적용될 재작성 목록을 보인다(MASK·FILTER·LIMIT 각 1건 이상) | E2E |
| M3-3 | 실행 결과의 열·행·행수·소요가 서버 응답과 일치하고 **재작성 SQL을 토글로** 볼 수 있다 | E2E |
| M3-4 | 거버넌스 상한이 자른 실행에서 경고 줄이 나오고, `moreRowsExist`의 **세 상태가 구분된다** | E2E(`LIMIT 0` → "확인하지 않음") |
| M3-5 | 차단·오류 실행에서 **서버의 분류 코드가 화면에 나온다** | E2E(미검토 403 · 룰 422) |
| M3-6 | 쿼리 상세 실행 이력이 `GET /api/queries/{id}/executions`를 쓰고 일반 사용자에게 `errorDetail`이 없다 | E2E(역할 2계정) |
| M3-7 | 실행 감사 화면이 **커서로 200건 밖 옛 기록에 도달**하고 전체 건수를 보인다 | E2E(`X-QG-Audit-Total`) |
| M3-8 | **결과 행이 storage에 없다** (F1) | E2E에서 `localStorage`·`sessionStorage` 전수 스캔 |
| M3-9 | 데스크톱 픽셀 불변 — spec 009의 4뷰포트 32검사가 그대로 통과 | 기존 Playwright 스냅샷 |

**M3-8·M3-9는 "새 기능이 기존 약속을 깨지 않았다"를 재는 축이다.** 나머지는 새 기능을 잰다.
...
> ⚠️ 2번의 귀속이 틀렸다 — §0 정정 참조. `(파생·AI)` 표시가 4번부터만 붙어 있어 1~3번이
> 사용자 결정으로 읽혔고, 그중 2번은 **AI 유추**였다.

1. **동일 DB로 시작** — 설정 DB에 데모 데이터 테이블. 원격 대상 DB·자격증명은 ④에서 재검토.
2. ~~**자동 재작성**~~ — **(파생·AI, 2026-07-27 정정)** MASK 치환·FILTER 주입을 서버가 수행.
   디자인의 마스킹 결과를 실제로 동작시키려면 서버가 SQL을 고쳐야 한다고 **AI가 유추**한 것이다.
   사용자 모델은 "판정하고 제안한다"이며 서버가 사용자의 SQL을 바꾸지 않는다(§0).
