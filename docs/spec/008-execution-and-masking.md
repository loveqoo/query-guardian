...
### 2.6 형식 검사 (신설 — 다른 결함의 폭발 반경을 줄이므로 M1보다 **선행**)
...
M0(형식 검사 + 실행 격리)은 **구현·검증 완료**. 구현 중 스펙에서 갈라진 결정과, 2차 적대 검토가 실측으로
잡은 것들:

**스펙에서 갈라진 결정(의도적)**
1. **형식 검사는 lint·저장 시점에도 발화한다** — §2.6은 실행 게이트에 뒀지만 *실행 대상 = 저장된 쿼리*라 두 집합이
   같다. 저장 때 잡으면 같은 방어를 유지하면서 "승인까지 받았는데 실행 불가"가 없어진다. **단락시키지 않고
   추가 위반**으로 넣어 나머지 룰 판정 결과를 함께 보여준다.
2. **파싱은 1회** (`DialectParser.inspect(sql) → (ParseResult, List<FormViolation>)`) — 형식 검사용 2차 파싱은
   타임아웃 레이스로 **판정 대상과 검사 대상이 갈라짐이 실측**됐다(84회 중 57회 fail-open 관측). M1의 재작성
   파싱도 이 경로에 얹는다.
3. **검사 불가 = 위반**(`UNVERIFIABLE`) — 빈 목록은 "위반 없음"으로 읽혀 형식 검사를 독립 단계로 호출하는
...
4. **`--`는 MySQL 규칙(뒤에 공백류/문장 끝)을 따른다** — Druid 프린터가 `- -1`을 `--1`로 출력하므로,
   무조건 거부하면 `form(sql) 통과 ⟹ form(rewrite(sql)) 통과`가 깨져 §3.0.3 재작성 검증이 성립하지 않는다.
...
- **왕복 정합성 성립**: 160개 입력(산술·리터럴·캐릭터셋 도입자·헥스/비트·인용 식별자·CTE·UNION·윈도우·
  `INTERVAL`·`COLLATE`·JSON 연산자·`CAST`·정렬·프린터 정규화 등)에서 `checkForm(sql)` 통과 ⟹
  `checkForm(print(parse(sql)))` 통과의 반례 **0건**. §3.0.3의 전제가 확인됐다.
  → 축약 코퍼스를 `parser/FormRoundTripTest`로 **영구 회귀**로 고정(Druid를 쓰므로 parser 테스트 패키지).
...
- 식별자 형식 검사: `catalog_column.name`·`demo_table_map.physical_name`에 정규식 검증
...
| M1-5 | 재작성 자체 검증(§3.0.3) 5항목 + **형식 재검사**(왕복 성질, M0에서 실측 확인) → 실패 시 실행 거부·부분 적용 금지 | 검증 실패 케이스가 예외 없이 차단 |
...
**OFFSET 처리 (2026-07-25 사용자 결정: 금지)**: 형식 검사에 `LIMIT_OFFSET_NOT_ALLOWED`를 추가해 거부한다.
...
**타사 모델(정합성 축)**
- 왕복 정합성 성립(160개 입력, 반례 0) → `parser/FormRoundTripTest`로 영구 고정.
...
**게이트 순서 (§5 확정)**
```
인증(401) → 데이터 권한 **현재 기준 재검사**(403) → 형식 검사(422) → 룰 **재판정**(422)
...
```
인증(401) → 데이터 권한(403, **현재 권한**으로 재검사) → 형식 검사(§2.6, 422)
...
0. ~~**M0 (선행)**: 형식 검사(§2.6) + 실행 격리 3종(§2.7 스키마·계정·매핑 총체성)~~ — **완료(§2.8 참조)**
...
7. (v2·적대 검토) **형식 검사**(주석 전면 금지·문형 허용목록·변수/한정자/0-테이블 거부)를 M0로 선행.
...
12. (M1 계획·사용자) **OFFSET 금지** — `LIMIT 1000,1000`으로 행 상한을 무한 우회할 수 있고, 상한의 의미를
    "이 실행으로 나간 행 수"로 고정해야 감사가 총 반출량을 셀 수 있다. 형식 검사에서 거부(`LIMIT_OFFSET_NOT_ALLOWED`).
