...
- 008 쿼리 실행·마스킹 재작성 — IR→SQL 재작성(MASK 치환·FILTER/LIMIT 주입, 전 스코프·fail-closed)+실행(동일 DB 데모 데이터, 읽기 전용·행 상한·타임아웃)+검토 승인 게이트+실행 감사; 실행 대상은 저장·승인된 쿼리 [실행, 마스킹, 재작성, 위생 게이트, 실행 격리, 완전 재판정] (v2 — 착수 전 §2.5~2.7 필독)
