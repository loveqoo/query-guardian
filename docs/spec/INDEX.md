...
- 007 인증(세션)·접근 권한 — actor 스텁→세션 principal(헤더 400 거부), 역할 3종 매트릭스, DB/테이블 allow를 lint·저장·요청 생성 전 경로 강제; 승인자는 app_user 승계, 권한 변경 비소급 [인증, 세션, 역할, 권한 게이트, actor 폐기, 판정 무권한 불변식] (v2 승인)
